...
public class CounterTest {
	...
	@Inject
	@Synchronous
	ImmediateCounterExecutor synchronousCounterExecutor;
	@Inject
	@Asynchronous
	AsyncCounterExecutor asynchronousCounterExecutor;
	@Inject
	...
	@Inject
	MoviesConfiguration config;

	@Deployment
	public static JavaArchive createDeployment() {
		JavaArchive jar = ShrinkWrap.create(JavaArchive.class).addPackages(true, Counter.class.getPackage())
				.addPackages(true, Asynchronous.class.getPackage())
				.addPackages(true, MoviesConfiguration.class.getPackage()).addClass(MutableInt.class)
				.addClass(Mutable.class).addAsResource("config.yaml", "config.yaml")
				.addAsResource("log4j2.xml", "log4j2.xml").addAsManifestResource(EmptyAsset.INSTANCE, "beans.xml");
	...
	public void testCounterServiceSynchronously() throws Exception {
		...
		CounterService counterService = CounterService.initialize(synchronousCounterExecutor, counterFactory,
	...
	public void testCounterServiceAsynchronously() throws Exception {
		...
		CounterService counterService = CounterService.initialize(asynchronousCounterExecutor, counterFactory,
				counterConfiguration);
		...
		Files.list(new File(counterConfiguration.getCountersFilesystemPath()).toPath()).forEach(path -> {
			try {
				...
				if (!fileName.startsWith(ASYNC_TEST_COUNTER_PREFIX))
					return; // not the right file, skip
		...
		// all counters sum value must equal to the number of time the increaseCounter
		// was called
	...
	public static void cleanup() {
		...
		String path = String
				.valueOf(((Map) ((Map) ((Map) config.get("movies")).get("counters")).get("file-system")).get("path"));
